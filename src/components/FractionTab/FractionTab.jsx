        ...
        {problems.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="nl-track-wrap">
                <div className="nl-track">
                  {Array.from({ length: totalTicks }, (_, pos) => {
                    ...
                    const numVal   = isStart ? null : (answers[`${i}_${blankIdx}_n`] ?? '')
                    const denomVal = isStart ? null : (answers[`${i}_${blankIdx}_d`] ?? '')
                    const blankCorrect = !isStart && checked && isBlankCorrect(p, i, blankIdx, answers)
                    const blankWrong   = !isStart && checked && !isBlankCorrect(p, i, blankIdx, answers)
                      ...
                      <div key={pos} className="nl-tick">
                        ...
                        <div className="nl-frac">
                          {isStart ? (
                            ...
                            <div className="nl-fraction-inputs" style={{ borderColor: blankCorrect ? '#1DD1A1' : blankWrong ? '#FF6B6B' : color, color: blankCorrect ? '#1DD1A1' : blankWrong ? '#FF6B6B' : '#fff' }}>
                              <input
                                className="nl-input"
                                type="number"
                                value={numVal}
                                ...
                                onChange={e => onAnswer(`${i}_${blankIdx}_n`, e.target.value)}
                              />
                              ...
                              <input
                                ...
                                placeholder="?"
                                disabled={submitted || isFC}
                                onChange={e => onAnswer(`${i}_${blankIdx}_d`, e.target.value)}
                              />
                            </div>
                          )}
                        </div>
              ...
              {ok && isFC  && <div className="nl-feedback" style={{ color: '#1dd1a1' }}>✅ Correct!</div>}
              {ok && !isFC && checked && <div className="nl-feedback" style={{ color: '#54a0ff' }}>✅ +0.5 correction</div>}
              {bad && <div className="nl-feedback" style={{ color: '#ff6b6b' }}>❌ Check the highlighted boxes above</div>}
            </div>
          )
        })}
      </div>
      ...
      <div className="actions">
        {!checked ? (
          ...
          <div className="result-panel">
            ...
            <button className="btn-check" style={{ marginBottom: '10px' }} onClick={onSubmit}>
              Submit Results 📬
            </button>
            <br />
