...
export default function App() {
  ...
  const editNLAnswers = useCallback(() => {
    setNlChecked(false)
    setNlFirstCorrect([])
  }, [])
  ...
  const handleCheck = useCallback((probs, ans) => {
    const correct = probs
      .map((p, i) => (parseInt(ans[i]) === p.answer ? i : -1))
      .filter(i => i >= 0)
  ...
  const handleNLCheck = useCallback((probs, ans) => {
    const correct = probs
      .map((p, i) => (isProblemCorrectNL(p, i, ans) ? i : -1))
      .filter(i => i >= 0)
  ...
  const handleSubmit = useCallback((probs, ans, fc, tabName, modeName) => {
    const corrections = probs.filter(
      (p, i) => !fc.includes(i) && parseInt(ans[i]) === p.answer,
    ).length
    const finalScore = fc.length + corrections * 0.5
    saveSession({
      tab: tabName,
      mode: modeName,
      firstCorrect: fc.length,
      corrections,
      finalScore,
      total: probs.length,
    })
  ...
  const handleNLSubmit = useCallback((probs, ans, fc) => {
    const corrections = probs.filter(
      (p, i) => !fc.includes(i) && isProblemCorrectNL(p, i, ans),
    ).length
    const finalScore = fc.length + corrections * 0.5
    saveSession({
      tab: 'math',
  ...
  return (
    <div className='app'>
      ...
      <div className='main-tabs'>
        ...
        <button
          className={`main-tab${tab === 'tables' ? ' main-tab--active' : ''}`}
      ...
      {tab === 'math' && (
        <MathTab
          ...
          onSubmit={() =>
            handleSubmit(problems, answers, firstCorrect, 'math', mode)
          }
          onEditAnswers={editAnswers}
          onNlAnswer={(key, val) =>
            setNlAnswers(prev => ({ ...prev, [key]: val }))
          }
